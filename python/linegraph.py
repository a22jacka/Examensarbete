...
files = glob.glob(f"{PATH_TO_SRC}*{("get" if IS_GET else "post")}-{vus}vu{(f"-{data}" if IS_GET else "")}-9500.csv")
...
plt.tight_layout()
plt.savefig(f"{PATH_TO_SAVE}{("get" if IS_GET else "post")}-{vus}vu{(f"-{data}" if IS_GET else "")}-9500.png")
plt.show()